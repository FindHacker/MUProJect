...
@property (weak, nonatomic) IBOutlet UIView *myPersonInfoView;
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *personInfoHeight;
@property (weak, nonatomic) IBOutlet UIView *personInfoView;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *showViewConstant;
@property (weak, nonatomic) IBOutlet UIView *showView;

@property (nonatomic) BOOL isShowPersonInfoView;

@property (nonatomic,strong) PPRevealSideViewController *prevealSideViewController;
@end

@implementation AdmissionRecordsViewController
//-(PPRevealSideViewController *)prevealSideViewController
//{
//    if (!_prevealSideViewController) {
//        
//        _prevealSideViewController = [[PPRevealSideViewController alloc] initWithRootViewController:self.navigationController];
//    
//    }
//    return _prevealSideViewController;
//}
- (IBAction)myButton:(UIButton *)sender {
    
    if (self.personInfoHeight.constant > 88) {
        self.personInfoHeight.constant = 40;
        self.showViewConstant.constant = 0;
        self.isShowPersonInfoView = NO;
        self.showView.hidden = YES;

    }else {
        self.personInfoHeight.constant = 117;
        self.showViewConstant.constant = 79;
        self.isShowPersonInfoView = YES;
        
        self.showView.hidden = NO;
    }
    
    [self.personInfoView setNeedsUpdateConstraints];
...
- (IBAction)commitButton:(UIBarButtonItem *)sender
{
    ...
    if([self.recordCase.caseType isEqualToString: @"未创建"]){
        createdTime = [self changeDateToDateString:[NSDate date]];
    }
    ...
    if (resultA.count == 1) {
        ...
        for (Node *node in parentNode.nodes.array) {
            [dict setObject:node.nodeContent==nil? @"":node.nodeContent forKey:node.nodeName];
...
//        @property (nonatomic, retain) NSDate * lastModifyTime;
        ...
        NSString *caseContent =[self convertJSONDataToString:[self convertToJSONDataFromList:dict]];
        NSString *lastModifyTime =[self changeDateToDateString:[NSDate date]];
        Patient *patient = self.recordCase.patient;
        
        NSString *pName = patient.pName;
        NSString *pID = patient.pID;
        
        NSDictionary *tempDic = NSDictionaryOfVariableBindings(caseType,caseState,lastModifyTime,pName);
        
        NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithDictionary:tempDic];
        
        [dict setObject:caseType forKey:@"caseType"];
        [dict setObject:caseState forKey:@"caseState"];
        [dict setObject:lastModifyTime forKey:@"lastModifyTime"];
        [dict setObject:pName forKey:@"pName"];

        if (pID) {
            [dict setObject:pID forKey:@"pID"];
        }
        [dic setObject:dict forKey:caseContent];
        if (createdTime) {
            [dict setObject:createdTime forKey:@"createdTime"];
        }
        
        [MessageObject messageObjectWithUsrStr:@"1" pwdStr:@"test" iHMsgSocket:self.socket optInt:20001 dictionary:[NSMutableDictionary dictionaryWithDictionary:@{@"bl":dict}] block:^(IHSockRequest *request) {
            if ([request.responseData isKindOfClass:[NSArray class]]) {
                NSArray *tempA = (NSArray*)request.responseData;
                NSDictionary *tempDic = (NSDictionary*)[tempA firstObject];
                
                if ([tempDic.allKeys containsObject:@"_id"]) {
                    NSString *caseID = tempDic[@"_id"];
                    [dic setObject:caseID forKey:@"caseID"];
...
- (void)viewDidLoad {
   ...
   // self.personInfoHeight.constant = 0;
    //[self.personInfoView setNeedsUpdateConstraints];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WriteCaseViewController *writeCaseVC = [storyboard instantiateViewControllerWithIdentifier:@"writeCaseViewController"];
    //[self.view addSubview:self.revealSideViewController.view];
    [self.revealSideViewController pushViewController:writeCaseVC onDirection:PPRevealSideDirectionBottom withOffset:200 animated:YES];
